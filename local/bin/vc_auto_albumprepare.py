...
def vc_auto_albumprepare(path: Path) -> None:
    ...
    for entry in path.iterdir():
        ...
        print(f'info: processing directory: {entry_name}', file=sys.stdout)

        entry_suffix = entry_name.removeprefix(_directory_prefix)
        if not entry_suffix.isdigit():
            continue

        print(f'info: apply discnumber tag: {entry_suffix}', file=sys.stdout)
...
def main(args: list[str]) -> int:
    ...
    try:
        ...
        print(f'error: failed to auto albumprepare: {work_dir.name}: {exc}', file=sys.stderr)
