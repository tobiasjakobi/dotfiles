...
[--check-mode=<check mode>] [--verbose] [--help]', file=sys.stdout)
...
def main(args: list) -> int:
    ...
    for _o, _a in opts:
        if _o in ('-h', '--help'):
            ...
            try:
                ...
                print(f'error: invalid album ID: {_a}: {err}', file=sys.stderr)

                return 2
        elif _o in ('-l', '--language'):
            if len(_a) <= 1:
                print(f'error: invalid language: {_a}', file=sys.stderr)
            ...
            if _a not in ('no-check', 'file-prefix', 'tag'):
                print(f'error: invalid check mode: {_a}', file=sys.stderr)
    ...
    if len(oargs) != 0:
        print(f'error: excessive arguments found: {oargs}', file=sys.stderr)

        return 5

    if album_id is None:
        print('error: album ID argument missing', file=sys.stderr)
    ...
    if language is None:
        ...
        print(f'info: using default language: {language}', file=sys.stdout)

    if check_mode is None:
        check_mode = CheckMode.NoCheck

        print(f'info: using default check mode: {check_mode.name}', file=sys.stdout)
    ...
    try:
        ...
        print(f'error: failed to decode album JSON: {err}', file=sys.stderr)

        return 8

    if not 'discs' in album:
        print('error: no discs found in album', file=sys.stderr)

        return 9

    if 'name' in album and verbose:
        album_name = album['name']
        print(f'info: album ID {album_id} has name: {album_name}', file=sys.stdout)

    try:
        discs = [InternalDisc.from_json(arg) for arg in album['discs']]

    except (KeyError, ValueError) as err:
        print(f'error: failed to parse discs: {err}', file=sys.stderr)
    ...
    print('info: available languages:', file=sys.stdout)
    for name in first_disc.tracks[0].names:
        if language == name.language:
            language_available = True

        print(f'\t{name.language}', file=sys.stdout)

    if not language_available:
        print(f'error: requested language not available: {language}', file=sys.stderr)
    ...
    if verbose:
        print('info: statistics:',  file=sys.stdout)
    ...
    if len(available_files) != total_tracks:
        print(f'error: total number of tracks does not match available files {total_tracks}: {len(available_files)}', file=sys.stderr)
    ...
    try:
        ...
        print(f'error: failed to create tag descriptors: {err}', file=sys.stderr)
    ...
    if check_mode == CheckMode.NoCheck:
        ...
        if verbose:
            print('info: handling {} album'.format('multi-disc' if is_multi_disc else 'single-disc'), file=sys.stdout)
        ...
        for arg in available_files:
            _m = get_meta(arg)
            if _m is None:
                print(f'warn: skipping file with unknown prefix: {arg}', file=sys.stdout)
                continue

            if _m[0] is None and is_multi_disc:
                print(f'warn: skipping file without disc prefix: {arg}', file=sys.stdout)
                continue
            ...
            if is_multi_disc:
                ...
                if not found_desc:
                    print(f'warn: skipping file without descriptor: {arg}', file=sys.stdout)
            else:
                found_desc = False
                for _t in tag_descriptors:
