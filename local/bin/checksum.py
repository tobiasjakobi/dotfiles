...
def _usage(app: str):
    print(f'Usage: {app} --sha-scan|--sha-check|--sfv-check|--sfv-migrate', file=sys.stdout)
...
def _get_filebase(arg):
    ...
    if m3u is None:
        ...
        print(f'info: no M3U found: using filename: {filebase}', file=sys.stdout)
    else:
...
def _try_singlefile(arg):
    ...
    sfv = _parse_sfv(arg)
    if len(sfv) != 1:
        print('error: singlefile check failed: multiple entries in SFV', file=sys.stderr)

        return None

    ref_split = splitext(sfv[0][0])
    if ref_split[0] != split_base[0]:
        print('error: singlefile check failed: filename mismatch', file=sys.stderr)
...
def sha_scan(arg, out, external_list) -> int:
    ...
    if out is None:
        ...
        if exists(sha):
            print(f'error: directory already has a checksum: {input_directory}', file=sys.stderr)
    ...
    if not filelist:
        print('error: no files found to check', file=sys.stderr)
...
def sha_check(arg) -> int:
    ...
    if not isfile(input_file):
        print(f'error: sha_check: checksum file not found: {input_file}', file=sys.stderr)
    ...
    try:
        ...
        print(f'error: sha_check: checksum parsing failed: {exc}', file=sys.stderr)
    ...
    for f in listdir(base_directory):
        ...
        if len(filename_matches) != 1:
            print(f'error: sha_check: file without checksum: {f}', file=sys.stderr)
    ...
    print(f'info: successfully checked {len(sha_tuples)} files, {bytes_total} bytes total', file=sys.stdout)
...
def sfv_check(arg) -> int:
    ...
    if not isfile(input_file):
        print(f'error: sfv_check: checksum file not found: {input_file}', file=sys.stderr)
...
def md5_check(arg) -> int:
    ...
    if not isfile(input_file):
        print(f'error: md5_check: checksum file not found: {input_file}', file=sys.stderr)
    ...
    if not filelist:
        print('error: md5_check: no files found to check', file=sys.stderr)
...
def sfv_migrate(arg) -> int:
    ...
    if retval != 0:
        print(f'error: sfv_migrate: initial check failed: {input_file}', file=sys.stderr)
    ...
    if fileprefix is None:
        print(f'error: sfv_migrate: failed to find file prefix: {input_file}', file=sys.stderr)
    ...
    if not isfile(m3u_path):
        ...
        if filelist is None:
            print(f'error: sfv_migrate: no M3U could be located: {working}', file=sys.stderr)
    ...
    if isfile(md5_path):
        ...
        if retval != 0:
            print(f'error: sfv_migrate: MD5 check failed for: {md5_path}', file=sys.stderr)

            return 4

    sha_path = pjoin(working, fileprefix + '.sha')
    if exists(sha_path):
        print(f'error: directory already has a SHA checksum: {working}', file=sys.stderr)
    ...
    if retval != 0:
        print(f'error: aborting migration since SHA scan failed: {working}', file=sys.stderr)
...
def main(args: list[str]) -> int:
    ...
    for o, a in opts:
        if checksum_mode != None:
            print('error: multiple modes selected', file=sys.stderr)
    ...
    if checksum_mode is None:
        print('error: unknown or no mode selected', file=sys.stderr)

        return 3

    if len(oargs) != 1:
        print('error: checksum file / directory argument invalid or missing', file=sys.stderr)
    ...
    if retval != 0:
        print(f'error: checksum {checksum_mode} failed with return value {retval}', file=sys.stderr)
