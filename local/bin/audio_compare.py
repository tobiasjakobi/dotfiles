    ...
    if parsed_args.mode is not None and parsed_args.file is not None:
        ...
        if parsed_args.other_file is not None:
            ...
            if not other_file.is_file():
                print(f'error: invalid other file: {other_file}', file=sys.stderr)
        ...
        if parsed_args.mode == 'compare':
            if other_file is None:
                print('error: other file missing', file=sys.stderr)
            ...
            try:
                ...
                print(f'error: error comparing audio: {exc}', file=sys.stderr)

                return 4
            
            if not result:
                print('error: audio does not match', file=sys.stderr)
            ...
            try:
                ...
                print(f'error: error computing MD5: {exc}', file=sys.stderr)

                return 2

            print(f'info: audio MD5: {result.hex()}', file=sys.stderr)

        else:
            print(f'error: invalid mode select: {parsed_args.mode}', file=sys.stderr)
